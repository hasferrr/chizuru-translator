...
import { errorHandler, extractTokens, perApiKeyRateLimit } from './middleware'
...
const rateLimitTranslateFree = rateLimit(rateLimitObjectData)
const rateLimitExtractContextFree = rateLimit(rateLimitObjectData)
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (error instanceof Error) {
        logger.error(`Free translation request failed`, { ip: req.ip, error: error.message, stack: error.stack }) // Log stack trace
      } else {
        logger.error(`Free translation request failed`, { ip: req.ip, error: String(error) }) // Log unknown error
      }
      next(error)
    }
  })
...
app.post(
  '/api/stream/extract-context-free',
  rateLimitExtractContextFree,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      logger.info(`Free context extraction request`, { ip: req.ip })
      ...
      if (error instanceof Error) {
        logger.error(`Free context extraction request failed`, { ip: req.ip, error: error.message, stack: error.stack }) // Log stack trace
      } else {
        logger.error(`Free context extraction request failed`, { ip: req.ip, error: String(error) }) // Log unknown error
      }
      next(error)
    }
  })
